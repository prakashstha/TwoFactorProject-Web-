...
else if(strcasecmp($action, 'input_event') == 0){
	$file =  $_POST['filename']; //getFileName('input_event');
	$timeStamp = $_POST['eventTime'];
	//$syncTime = $_POST['syncTime'];
	$eventType = $_POST['eventType'];
	$fieldID = $_POST['fieldIDent'];
	$keyCode = $_POST['keyCode'];
	
	$fields = array($timeStamp, $eventType, $fieldID, $keyCode);
	$fp = fopen($file, 'a');
	if($fp){
		fputcsv($fp, $fields);
		fclose($fp);
